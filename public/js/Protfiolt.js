...
$(document).ready(function () {
  ...
  $("#CategoryTextBut").click(function () {
    if (!checkCategoryText($(".CategoryText").val()) && $(".CategoryText").val() != "") {
      $("#categoryTextLineList").append('<div class="badge bg-info text-wrap categoryTextLine"onclick="CategoryTextDel(' + $("#categoryTextLineList").children().length + ')" >' + $(".CategoryText").val() + '</div>');
    }
...
window.CategoryAdd = function ($CategoryIndex) {
  if (!checkCategoryText($(".dropdown-item").eq($CategoryIndex).text())) {
    $("#categoryTextLineList").append('<div class="badge bg-info text-wrap categoryTextLine" onclick="CategoryTextDel(' + $("#categoryTextLineList").children().length + ')" >' + $(".dropdown-item").eq($CategoryIndex).text() + '</div>');
  }
};
    ...
    success: function success(response) {
      ...
      if (response.error) {
        ...
        if (response.errors == "") {
          ...
          sortAboutmetag(); // location.href = '/managerpage'
        }
      }
    },
    cache: false,
